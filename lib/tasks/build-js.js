...
function processThemeJs() {
  ...
  return gulp.src([config.roots.js, '!' + config.roots.vendorJs]).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(gulp.dest(config.dist.assets)).pipe(ignore.include('**/*.js')).pipe(uglify({
    mangle: true,
    compress: true
  })).pipe(extReplace('.min.js', '.js')).pipe(gulp.dest(config.dist.assets));
}
