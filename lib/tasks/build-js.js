...
function processThemeJs() {
  ...
  return gulp.src([config.roots.js, '!' + config.roots.vendorJs]).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(gulp.dest(config.dist.assets)).pipe(uglify({
    mangle: true,
    compress: true
  })).pipe(extReplace('.min.js.liquid', '.js.liquid')).pipe(gulp.dest(config.dist.assets));
}
